...
public:
    ...
    ConcaveHull(const Eigen::MatrixPt &ipoints3d,
                const Eigen::Vector4d &planeEq);
    
    ConcaveHull(const std::vector<Polygon_2> &polygons,
                    const std::vector<Eigen::MatrixPt> &polygons3d,
    ...
    void init(const Eigen::MatrixPt &ipoints3d,
              const Eigen::Vector4d &planeEq);
    ...
    const std::vector<Eigen::MatrixPt> &getPolygons3d() const {
        return polygons3d;
    }
    ...
    ConcaveHull intersect(const std::vector<Eigen::MatrixPt> &otherPolygons3d,
                          double areaThresh = 0.05) const;
    
    ConcaveHull clipToCameraFrustum(const Eigen::Matrix3d &K,
                                    int rows,
                                    int cols,
                                    double minZ);
...
private:
    ...
    double totalArea;
    std::vector<Eigen::MatrixPt> polygons3d;
